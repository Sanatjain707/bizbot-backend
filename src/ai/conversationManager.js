  ...
  try {
    ...
    if (slot.intent === 'reschedule') {
      ...
      if (existing) {
        ...
        if (r.rescheduled) console.log('🔄 Appointment rescheduled')
        else console.warn(`⚠️ Reschedule rejected (${r.code}): ${r.reason}`)
        return
      }
      // No existing booking → fall through and treat as a fresh booking.
    }

    // ── Same-service update guard (still treat as reschedule) ──
    const existing = await getUpcoming(customer.id)
    if (existing && existing.service === slot.service) {
      const r = await rescheduleBooking({
        business, customer, dateISO: slot.date, hhmm: slot.time, service: slot.service,
      })
      if (r.rescheduled) console.log('🔄 Updated existing appointment')
      else console.warn(`⚠️ Update rejected (${r.code}): ${r.reason}`)
      return
    }
